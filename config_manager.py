...
class ConfigManager:
    ...
    def _load_env_configs(self) -> Dict[str, dict]:
        account_text = os.environ.get("KuroBBSAccount", "")
        env_configs = {}

        if not account_text.strip():
            return env_configs

        for index, line in enumerate(account_text.splitlines(), start=1):
            line = line.strip()
            if not line:
                continue
            ...
            user_name = parts[1] if len(parts) > 1 and parts[1] else f"env_{index}"
            if self.config_prefix and not user_name.startswith(self.config_prefix):
                continue

            wwrole_id = parts[2] if len(parts) > 2 else ""
            eeerole_id = parts[3] if len(parts) > 3 else ""
            user_id = parts[4] if len(parts) > 4 else ""
            devcode = parts[5] if len(parts) > 5 else ""
            distinct_id = parts[6] if len(parts) > 6 else ""
            retry_times = parts[7] if len(parts) > 7 and parts[7] else 3
            auto_reple_sign = (
                self._parse_bool(parts[8]) if len(parts) > 8 else False
            )
            completed = bool(devcode and distinct_id and user_id)

            env_configs[user_name] = {
                "token": token,
                "enable": True,
                "completed": completed,
                "auto_reple_sign": auto_reple_sign,
                "retry_times": retry_times,
                "game_info": {
                    "distinct_id": distinct_id,
                    "devcode": devcode,
                    "wwroleId": wwrole_id,
                    "eeeroleId": eeerole_id,
                },
                "user_info": {
                    "userId": user_id,
                },
        ...
        if env_configs:
            log_info(f"从环境变量 KuroBBSAccount 加载 {len(env_configs)} 个账号配置")
    ...
    def load_user_config(self, user_name: str) -> Optional[UserConfig]:
        ...
        if not os.path.exists(config_path):
            if user_name in self.env_configs:
                config = UserConfig.from_dict(user_name, self.env_configs[user_name])
                log_debug(f"成功加载环境变量配置: {user_name}")
                return config
    ...
    def save_user_config(self, config: UserConfig) -> bool:
        ...
        try:
            ...
            if config.name in self.env_configs:
                self.env_configs[config.name] = data
                log_debug(f"成功更新环境变量配置: {config.name}")
                return True
    ...
    def list_all_configs(self) -> List[str]:
        ...
        try:
            ...
            if not user_names and self.env_configs:
                user_names = list(self.env_configs.keys())
                log_info(f"未找到 YAML 配置文件，使用环境变量中的 {len(user_names)} 个账号")
            return user_names
