...
GitHub Actions 适合没有服务器的用户，支持定时运行且配置简单。
...
| 名称 | 必填 | 说明 | 示例 |
| ---- | ---- | ---- | ---- |
| `KUROBBSACCOUNT` | 是 | 账号信息 | `token` (最简模式) 或 `token#用户名#...` |
| `PUSH_SETTING_ENABLE` | 否 | 是否开启推送 | `true` |
| `PUSH_SETTING_PUSH_SERVER` | 否 | 推送服务名 | `pushplus` 或 `dingrobot` (支持多个，用逗号分隔，如 `pushplus,dingrobot`) |
| `PUSH_SETTING_PUSH_LEVEL` | 否 | 推送详细程度 | `1` (1=总结, 2=一条详细, 3=多条详细) |
| `PUSH_SETTING_PUSH_TOKEN` | 否 | 推送 Token | `XXXXXX` (适用于 pushplus/server酱等) |
| `PUSH_DINGROBOT_WEBHOOK` | 否 | 钉钉 Webhook | `https://oapi.dingtalk.com/robot/send?access_token=XXX` |
| `PUSH_DINGROBOT_SECRET` | 否 | 钉钉加签密钥 | `SECxxx` (如果钉钉设置了加签则必填) |

> [!IMPORTANT]
> **关于 KUROBBSACCOUNT 的填写：**
> - **最简模式（推荐）**：只需填入 `token` 即可。脚本会自动获取你的用户名、鸣潮 ID、战双 ID 等信息。
> - **完整模式**：`token#用户名#鸣潮ID#战双ID#用户ID#设备码#唯一标识#重试次数#自动补签`。
> - 如果你有多个账号，请换行输入。

> [!TIP]
> 如果使用其他推送服务（如 Telegram），请根据环境变量规则添加 Secrets。
> 格式为 `PUSH_{SECTION}_{OPTION}`，例如 Telegram 的 `bot_token` 为 `PUSH_TELEGRAM_BOT_TOKEN`。

#### 二、启用工作流

1. 点击仓库顶部的 **Actions** 标签。
2. 在左侧选择 **Kuro Auto Signin**。
3. 如果看到 "Workflows are not run on duplicated repositories"，点击 **I understand my workflows, go ahead and enable them**。
4. 点击 **Run workflow** -> **Run workflow** 手动触发测试。

#### 三、定时运行

工作流默认每天北京时间 **08:30** 自动运行。你可以通过修改 [.github/workflows/run.yml](.github/workflows/run.yml) 中的 `cron` 表达式来调整时间。
